...
public class Odometer{
  ...
  // TODO
   ...
   public Pose2d cameraToCenter(Pose2d cameraPose) {
    return new Pose2d(new Translation2d(cameraPose.getX(), cameraPose.getY()), cameraPose.getRotation());
   }
   ...
   public void updateOdometry () {
        ...
        if(limelight.hasTarget()){
            if(Math.abs(getPoseX()-limelight.getX())<Constants.VisionConstants.ALLOWED_POSE_ERROR_IN_METERS || Math.abs(getPoseY()-limelight.getY())<Constants.VisionConstants.ALLOWED_POSE_ERROR_IN_METERS){
                odometry.addVisionMeasurement(limelight.getFieldPose(), Timer.getFPGATimestamp());
            }
        }    
